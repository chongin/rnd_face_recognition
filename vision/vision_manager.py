...
from emotion_detector_machine_learning import EmotionDetectorMachineLearning
...
class VisionManager:
    def __init__(self) -> None:
        ...
        self.emotion_detector = EmotionDetectorMachineLearning()
        self.emotion_detector.init_algorithm_models()
        self.emotion_detector.init_machine_learning_model()
    ...
    def excute_frame(self, frame) -> None:
        ...
        if self.face_detection_flag:
            predict_faces = self.face_detector.predict_faces(frame)
            self.face_detector.draw_face_rectangle(frame, predict_faces)

        if self.emotion_detection_flag:
            predict_emotions = self.emotion_detector.predict_emotions(frame)
