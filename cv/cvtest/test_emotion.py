...
emotion_recognition_model = pickle.load(open('/Users/chongin/rnd_work/rnd_face_recognition/vision/trained_models/machinelearning_face_emotion.pkl',mode='rb'))
    ...
    if len(detections) > 0:
        for i , confidence in enumerate(detections[0,0,:,2]):
            if confidence > 0.5:
                ...
                print(f"names: {emotion_recognition_model.predict(vectors)}")
                emotion_score = emotion_recognition_model.predict_proba(vectors).max()
                print(f"scores: {emotion_recognition_model.predict_proba(vectors)}")
...
# img, results = pipeline_model('../face/images/Chong In Ng/selfie.jpg')
...
while True:
    ...
    img, results = pipeline_model(flip_frame)
    cv2.imshow('detection', img)
